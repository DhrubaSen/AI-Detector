...
def is_poetry(text: str) -> dict:
    ...
    # Anaphora — lines starting with same word (common in human poetry)
    ...
    # Rhyme detection — end words rhyming = strong human poetry signal
    # Check if last words of consecutive lines rhyme (share last 2+ chars)
    ...
    poetry_score = sum([
        ...
        has_rhyme,      # rhyming couplets = strong human signal
        has_anaphora,   # repeated line openings = human poetry device
    ])
...
def analyse_text(text: str) -> dict:
    ...
    if poetry.get("is_poetry"):
        ...
        # Neutralise sentence length uniformity penalty — rhyming couplets are uniform
        ...
        # Rhyme = strong human poetry signal
        if poetry.get("has_rhyme"):
            ai_probability = max(0, ai_probability - 0.20)
            if poetry.get("rhyme_pairs", 0) >= 4:
                ai_probability = max(0, ai_probability - 0.10)  # extra for rich rhyme
